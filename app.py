...
# 1. FUNÇÃO DE FORMATAÇÃO (Espaçamento Corrigido)
def format_br(val):
    if val is None: return "R$ 0,00"
    # Adicionado um espaço após o R$ para clareza
...
# 3. ESTILIZAÇÃO CSS (Kerning Corrigido)
st.markdown("""
    <style>
    ...
    .metric-value { 
        ...
        font-weight: 800; 
        margin-top: 10px; 
        letter-spacing: 0px; /* CORREÇÃO: Removido o aperto das letras */
        line-height: 1.1; 
        color: #000;
        display: block;
    }
...
# 5. TENTATIVA DE CONEXÃO GSHEETS
try:
    conn = st.connection("gsheets", type=GSheetsConnection)
    # Se estiver no step 0, tenta ler se já existe configuração
    if st.session_state.step == 0:
        check_df = conn.read(worksheet="Config", usecols=[0,1], ttl=0)
        if not check_df.empty:
            st.session_state.step = 4
            # Mapeia os dados da planilha para o estado
    ...
    # Se falhar a conexão, apenas ignora e segue para o onboarding manual
    pass
...
if st.session_state.step == 0:
    ...
    v_total = st.number_input("SALDO TOTAL HOJE", min_value=0.0, format="%.2f")
    v_reserva = st.number_input("RESERVA ESTRATÉGICA", min_value=0.0, format="%.2f")
